...
int deck_contains(deck_t * d, card_t c) {
  ...
  for(i=0; i<(d->n_cards);i++){
    if(suit_letter(**ptr) == suit_letter(c) &&
       value_letter(**ptr) == value_letter(c))
      return 1;
    ptr++;
  }
  return 0;
...
void add_card_to(deck_t *deck, card_t c){
  ...
  deck->cards[deck->n_cards-1] = realloc (deck->cards[deck->n_cards-1],sizeof
					  (*(deck->cards[deck->n_cards-1])));
